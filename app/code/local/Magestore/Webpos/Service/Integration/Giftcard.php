...
//            Mage::app()->setCurrentStore($curent_store_id);
    ...
    public function createGiftCard($couponCode, $amount, $status) {

        $message = array();
         $data = array();
        if(!$couponCode) {
            $message[] = $this->_helper->__('Please fill gift code field.');
            $status = Magestore_Webpos_Api_ResponseInterface::STATUS_ERROR;
        ...
        if($amount <= 0) {
            $message[] = $this->_helper->__('The price is incorrect.');
            $status = Magestore_Webpos_Api_ResponseInterface::STATUS_ERROR;
            
             return $this->getResponseData($data, $message, $status);
        }

        $model = Mage::getModel('giftvoucher/giftvoucher')->load($couponCode, 'gift_code');
        ...
        if(!$model->getId()) {
            ...
            $product
                ...
                ->setAttributeSetId(4)
                ->setTypeId('giftvoucher') 
                ->setCreatedAt(strtotime('now')) 
                ->setSku($couponCode)
                ->setName('Giftcard '.$couponCode)
                ->setStatus(1) 
                ->setTaxClassId(0)
                ->setVisibility(Mage_Catalog_Model_Product_Visibility::VISIBILITY_BOTH)
                ->setGiftType(1)
                ->setGiftValue($amount)
                 ->setGiftTemplateIds(array(1))
                 ->setGiftPriceType(1)
                   ->setStockData(array(
            ...
            $warehousePrimary = Mage::helper('inventoryplus/warehouse')->getPrimaryWarehouse();
            $warehousePrimary->updateStock($product->getId(), 1, 1, true);
            ...
            $message[] = $this->_helper->__('Gift card "%s" has been created successfully.', $couponCode);
            $status = Magestore_Webpos_Api_ResponseInterface::STATUS_SUCCESS;
           ...
           // $data['product_name'] = $product->getName();  
            ...
            $data['gift_code'] = $couponCode;
            $data['balance'] = $amount;
            $data['amount'] = $amount;
            $data['currency'] = 'DKK';
            $data['extra_content'] = Mage::helper('giftvoucher')->__('Created by webpos');
            ...
            */
                ...
                $status = Magestore_Webpos_Api_ResponseInterface::STATUS_ERROR;
                 ...
                 $message[] = $this->_helper->__('Gift card "%s" has exist.', $couponCode);
                $status = Magestore_Webpos_Api_ResponseInterface::STATUS_ERROR;
            }
